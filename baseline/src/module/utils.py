...
def cuda_if_available(use_cuda: Optional[bool] = None) -> torch.device:
    ...
    _use_cuda = (use_cuda is None or use_cuda) and (mps_available or cuda_available)
    if use_cuda is True and not (mps_available or cuda_available):
        warnings.warn("Requested CUDA but neither CUDA nor MPS is available, running on CPU")
    if use_cuda is False and (mps_available or cuda_available):
        warnings.warn(
            "Running on CPU, even though CUDA or MPS is available. "
            "(This is likely not desired, check your arguments.)"
        )
    return torch.device("cuda" if cuda_available and _use_cuda else "mps" if mps_available and _use_cuda else "cpu")
